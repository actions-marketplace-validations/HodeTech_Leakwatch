...
func (v *Verifier) Verify(ctx context.Context, raw detector.RawFinding) finding.VerificationResult {
	...
	default:
		slog.ErrorContext(ctx, "gitlab verifier: unexpected status code",
...
func handleActiveToken(ctx context.Context, body io.Reader) finding.VerificationResult {
	...
	if err := json.NewDecoder(body).Decode(&user); err != nil {
		...
		return finding.VerificationResult{
			...
			Message: "GitLab token is active (could not parse user info)",
		}
	}
	...
	slog.InfoContext(ctx, "gitlab verifier: token is active",
